...
class Api {
  ...
  // RAPPELS
  // ===============================
  Future<List<dynamic>> getRappels() async {
    final response = await http.get(
      Uri.parse("$baseUrl/api/Rappel"),
      headers: headers(),
    );
  ...
  Future<bool> addRappel(Map<String, dynamic> rappel) async{
    final response = await http.post(
      Uri.parse("$baseUrl/api/Rappel"),
        headers: headers(),
      body: jsonEncode(rappel),
    );

    return response.statusCode == 200 || response == 201;
  }
